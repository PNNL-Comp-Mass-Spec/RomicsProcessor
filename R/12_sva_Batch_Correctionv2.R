...
#' This function is used to perform a ComBat batch correction on a romics_object using a factor of the romics_object as the batch correction covariate.
#'
...
RomicsBatchCorrection<-function(romics_object, batch_factor="factor", method="ComBat", ...){
  ...
  if(!"sva" %in% (.packages())){
    print("The package 'sva' was not loaded, it was loaded")
    library("sva")
    }
