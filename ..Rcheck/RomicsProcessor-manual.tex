...
emmeans, FactoMineR, factoextra, missMDA, uwot, Rtsne, igraph,
gplots, ComplexHeatmap, circlize, gridExtra, grid, patchwork,
FNN, matrixStats, ggsignif, ggtree}
\item[Depends]\AsIs{R (>= 4.0.0)}
\item[Suggests]\AsIs{pmartR, sva}
\end{description}
\Rdcontents{Contents}
\HeaderA{RomicsProcessor-package}{RomicsProcessor: An R package for analyzing omics data}{RomicsProcessor.Rdash.package}
\aliasA{RomicsProcessor}{RomicsProcessor-package}{RomicsProcessor}
\keyword{internal}{RomicsProcessor-package}
%
\begin{Description}
...
Geremy Clair
\end{Author}
\HeaderA{romicsGlmBinomial}{romicsGlmBinomial()}{romicsGlmBinomial}
%
\begin{Description}
uses the function glm() to perform a glm binomial test to identify if the missingness of the values in the different groups is at random or not (performs all the paired tests between the groups of the selected factor). Results are recorded in the statistics layer of the outputted romics\_object.
\end{Description}
%
\begin{Usage}
\begin{verbatim}
...
\begin{Arguments}
\begin{ldescription}
\item[\code{romics\_object}] has to be an romics\_object created with the function romicsCreateObject(),

\item[\code{factor}] a character string indicating the factor to use for the test, the list of the available factor can be obtained by using the function romics\_factors(), if missing the function will use the main factor of the object

\item[\code{padj}] a logical variable indincating wheter to perform or not adjustment of pvalues

\item[\code{padj\_method}] correction method. Must be in  "holm", "hochberg", "hommel", "bonferroni", "BH", "BY", "fdr","none"

\item[\code{reverse\_order}] a boolean to indicate if the order of the factors needs to be reversed (this will make the calculated fold changes values A/B become B/A or log2(A/B) become log2(B/A))
\end{ldescription}
\end{Arguments}
%
\begin{Details}
This function performs a generalized linear model with binomial family to test whether missingness patterns differ significantly between groups. It tests all pairwise combinations of factor levels. Results include p-values, adjusted p-values, and directionality indicators.
\end{Details}
%
\begin{Value}
an romics\_object with the statistical layer containing the newly generated glm binomial test results
\end{Value}
%
\begin{Author}
Geremy Clair
...
Geremy Clair
\end{Author}
\HeaderA{romicsTtest}{romicsTtest()}{romicsTtest}
%
\begin{Description}
Performs all possible paired-T.tests for each variable using the levels of the selected factor of the romics\_object. The results are added as new columns in the statistics layers.
\end{Description}
%
\begin{Usage}
\begin{verbatim}
...
\item[\code{alternative}] a character string specifying the alternative hypothesis, must be one of "two.sided" (default), "greater" or "less". You can specify just the initial letter.

\item[\code{paired}] a logical indicating whether you want a paired t-test.

\item[\code{pairing\_factor}] name of a factor contained in an r\_object, the list of the available factor can be obtained by using the function romics\_factors()

\item[\code{var.equal}] a logical variable indicating whether to treat the two variances as being equal. If TRUE then the pooled variance is used to estimate the variance otherwise the Welch (or Satterthwaite) approximation to the degrees of freedom is used.
...
\item[\code{mode}] must be in "vs", "enrichments" indicates if the groups should be compaired by pair or against all other groups

\item[\code{percentage\_completeness}] a numerical value comprised between 0 and 100 to indicate the minimum completeness required in at least one group calculate the T.test (if set completeness is not met, p and fold change will be NA)

\item[\code{reverse\_order}] a boolean to indicate if the order of the factors needs to be reversed (this will make the calculated fold changes values A/B become B/A or log2(A/B) become log2(B/A))

\item[\code{...}] other parameters can be passed down to the t.test() function from the 'stat' package.
\end{ldescription}
\end{Arguments}
%
\begin{Details}
...
%
\begin{Value}
an romics\_object with the statistical layer containing the newly generated t.tests and fold-changes
\end{Value}
%
\begin{Author}
Geremy Clair
...
Geremy Clair
\end{Author}
\HeaderA{romicsWilcoxTest}{romicsWilcoxTest()}{romicsWilcoxTest}
%
\begin{Description}
Performs all possible paired Wilcoxon signed-rank test for each variable using the levels of the selected factor of the romics\_object. The results are added as new columns in the statistics layers.
\end{Description}
%
\begin{Usage}
\begin{verbatim}
...
\item[\code{mode}] 'vs' or 'enrichment' indicating if all the between group comparisons should be performed (vs) or if the features in a given group should be compared to all the other groups (enrichment).

\item[\code{...}] other parameters can be passed down to the wilcox.test() function from the 'stat' package.
\end{ldescription}
\end{Arguments}
%
\begin{Details}
...
%
\begin{Value}
an romics\_object with the statistical layer containing the newly generated Wilcox tests and fold-changes
\end{Value}
%
\begin{Author}
Geremy Clair
