...
#' @param scale_feature TRUE or FALSE, if TRUE scales features by rows (z-score normalization). Default: TRUE
#' @param feature_list Character vector of features to include in heatmap. If NULL, uses all features. Default: NULL
#' @param filter_by_stat_column Character string specifying the statistics column to filter features. If NULL, no filtering applied. Example: "Healthy_vs_Disease_Ttest_padj". Default: NULL
#' @param stat_column_filter Character string specifying the filter expression using operators <, >, <=, >=, ==, !=. Example: "<0.05" or ">1.5". Only used if filter_by_stat_column is specified. Default: NULL
#' @param gradient_colors Character vector of colors for the heatmap gradient. If NULL, uses viridis color scale. For 3 colors: c(low, mid, high). For 2 colors: c(low, high). Default: NULL
#' @param viridis_option Character string specifying the viridis color scale option. Only used if gradient_colors is NULL. Accepted values: "viridis", "plasma", "inferno", "magma", "cividis", "rocket", "mako", "turbo". Default: "viridis"
#' @param show_completeness TRUE or FALSE, if TRUE shows data completeness as tile transparency (alpha). If FALSE, completely missing data (0% completeness) is shown as black tiles. Default: TRUE
#' @param show_dendrogram TRUE or FALSE, if TRUE displays hierarchical clustering dendrogram on the left side of the heatmap. Requires ggdendro package. Default: TRUE
#' @param show_feature_names TRUE or FALSE, if TRUE displays feature names on the right side of the heatmap. Default: TRUE
#' @param show_clusters TRUE or FALSE, if TRUE displays cluster assignments as a sidebar. Default: FALSE
...
#'
#' If feature_list is provided, the heatmap will only display those features.
#' If filter_by_stat_column and stat_column_filter are both specified, features are filtered before visualization.
#' If gradient_colors is provided, viridis_option is ignored.
#' If show_clusters is TRUE and n_clusters is specified, the dendrogram is cut into clusters and a sidebar shows cluster assignments.
#'
#' @return A ggplot/cowplot object containing the heatmap visualization (with optional dendrogram and cluster sidebar).
#' Clustering results are attached as attributes (accessible via attr(plot, "clustering_results")).
#'
#' @author Geremy Clair
#' @export
...
romicsGLMheatmap <- function(romics_object,
  ...
  sidenote <- sidenote[do.call(order, sidenote), ]
  m <- m[order(match(rownames(m), rownames(sidenote))), ]
